...
## Goal
Fix SSO, websocket, and client issues that prevent chat from loading.

After you finish, you should be confident the task worked—not just that a page loaded.

## Prerequisites
- Note whether web, desktop, or mobile client fails
...
## Steps
1. Retry in a private browser window after Authelia re-login.
2. If the UI loads but messages never appear, corporate proxies may block websockets—try Tailscale or another network.
3. Update/reinstall the desktop client if only the native app fails.
4. Check https://status-style signals via staff if many users report the outage simultaneously.
5. Capture exact error text and file a ticket with Internal group for ops.

## Verify
Realtime messages appear without manual refresh.
